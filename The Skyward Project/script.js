(function () {
  ...
  /* ----------------------------------------------------------------------
     Fixed navigation.
     On phones the bar is always visible. On desktop it waits until the
     homepage opening screen has scrolled away.
     ---------------------------------------------------------------------- */
  ...
  } else if (nav && sentinel && supportsObserver) {
  ...
  if (supportsObserver && !reduceMotion) {
    ...
    /* Headlines arrive a word at a time on desktop only. On a phone,
       inline-block words overflow the screen and shove the layout left. */
    var splitHeadlines = !(
      window.matchMedia &&
      window.matchMedia("(max-width: 800px)").matches
    );
